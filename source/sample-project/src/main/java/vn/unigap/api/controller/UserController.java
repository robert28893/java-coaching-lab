...
public class UserController extends AbstractResponseController {
    ...
    @Operation(summary = "Lấy danh sách users", responses = {
            @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ResponsePageUser.class))) })
    ...
    @Operation(summary = "Lấy thông tin user theo id", responses = { @ApiResponse(responseCode = "200", content = {
            @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseUser.class)) }) })
    ...
    @Operation(summary = "Thêm mới user", responses = { @ApiResponse(responseCode = "201", content = {
            @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseUser.class)) }) })
    @PostMapping(value = "")
    ...
    @Operation(summary = "Cập nhật user", responses = { @ApiResponse(responseCode = "200", content = {
            @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseUser.class)) }) })
    @PutMapping(value = "/{id}")
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id,
            @RequestBody @Valid UpdateUserDtoIn updateUserDtoIn) {
        return responseEntity(() -> {
            return this.userService.update(id, updateUserDtoIn);
        });
    }

    @Operation(summary = "Xóa user", responses = { @ApiResponse(responseCode = "200", content = {
