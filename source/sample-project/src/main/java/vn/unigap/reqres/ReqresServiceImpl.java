...
public class ReqresServiceImpl implements ReqresService {
    ...
    @Autowired
    public ReqresServiceImpl(RestTemplate restTemplate, ReqresProperties properties) {
    ...
    public ReqresPageDtoOut<ReqresUserDtoOut> listUsers(int page, int perPage) {
        String url = UriComponentsBuilder.fromHttpUrl(this.properties.getUrl()).path("/api/users")
                .queryParam("page", page).queryParam("per_page", perPage).build().toString();
